...
1. What is the purpose of using _sessions_?
  ...
  - sessions allow for creating cookings onto the users client to make sure that the correct user is logged in. once session is gone the user is no longer logged on.

2. What does bcrypt do to help us store passwords in a secure manner.

  - bcrypt hashes the password so we do not have to store the password exposing it for anyone to exploit. hashes make it harder for a hacker to steal the password.

3. What does bcrypt do to slow down attackers?

  - hashing the password adds time to the password, so if the attacker would have to spend a lot of resources and time to crack the password which might not be worth it for the attacker.

4. What are the three parts of the JSON Web Token?

  - the 3 parts of a JSON web Token is the header, payload and signature.
